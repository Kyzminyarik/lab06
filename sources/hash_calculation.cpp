...
void get_hash(json_creator& hashes_json){
  while (in_process){
    std::string rand_str = std::to_string(std::rand());//формирование рандомной строки
    std::string hash = picosha2::hash256_hex_string(rand_str);//хэш генератор
    std::time_t timestamp(std::time(nullptr));

    if (hash.substr(hash.size()-4) == "0000" && //если на конце 4 нуля то м применяем уровень логирования инфо
...
void hash_calc(const std::string& path, size_t num_of_threads, size_t rotation){
  ...
  for (size_t i = 0; i < num_of_threads; ++i){
    threads.emplace_back(get_hash, std::ref(hashes_json));//чтобы каждый поток знал с какими данными он рабоатет
  }
